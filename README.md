...
현대차 범퍼 납품사 CAMS 대표와 임원을 위한 모니터링 대시보드. 현대차 수요(주가·뉴스), 환율, 범퍼 주원료 PP(폴리프로필렌)의 원가 선행지표를 매일 자동 수집·적재하고, 아침 브리핑 형태로 CAMS 사업 영향을 요약한다.

- **스택**: Next.js 15 (App Router) + Neon DB + Vercel Cron
- **공개용**: 비밀번호 없음. 개발/배포 링크를 공유받은 사람이 바로 접속 가능하며, robots noindex로 검색 비노출
...
2. 환경변수 등록: `DATABASE_URL`, `CRON_SECRET`, `GROQ_API_KEY`(AI 브리핑용, 선택)
...
- 인증: 대시보드는 비밀번호 없이 링크로 접속. cron만 `CRON_SECRET` Bearer로 보호
- 수집 실패 표시: 시세 마지막 적재일이 **2일 이상** 지나면 카드에 "수집 실패" 뱃지 (주말 휴장 1일은 정상 취급)
- AI 브리핑: cron 마지막 단계에서 Groq API로 1일 1회 생성 (시세 30일 + 뉴스 7일 제목 입력 → 오늘의 결론/주요 변화/확인할 뉴스/CAMS 인사이트). 실패해도 시세·뉴스 적재와 무관. 모델은 `GROQ_MODEL`로 교체 가능
- 화면 요약: 페이지는 기존 시세·뉴스·브리핑 조회 결과를 재사용해 `수요 신호`, `원가 신호`, `환율 신호`, `리스크 신호`와 데이터 기준 시각을 표시한다. 추가 DB 조회 없이 화면에서 사업 언어로 변환한다.
